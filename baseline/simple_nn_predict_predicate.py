...
from models import PredictPredicateNN
...
class BBoxDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        label = relationship_labels[label]
    ...
    for i, data in enumerate(data_loader):
      ...
      if torch.cuda.is_available():
        ims, subjs, objs, unions, labels = make_cuda(ims, subjs, objs, 
                                                     unions, labels)
      outputs = model(subjs, objs, unions)
      _,argmaxed_idx = torch.max(outputs, -1)
      total += labels.size(0)
      correct += int((argmaxed_idx.view(1, -1) == Variable(labels)).sum())
    acc = (correct + 0.0) / total
    print('Test Accuracy of the '+ dataset_name +' model: %f %%' % acc)
    return acc 

def collate_remove_none(batch):
  ims, subjs, objs, unions, labels = [], [], [], [], []
  for b in batch:
    if b is not None:
      ims.append(Variable(b[0]))
      subjs.append(Variable(b[1]))
      objs.append(Variable(b[2]))
      unions.append(Variable(b[3]))
      labels.append(b[4])
  labels = (Variable(torch.FloatTensor([labels])), torch.LongTensor([labels]))
  return torch.cat(ims), torch.cat(subjs), torch.cat(objs), torch.cat(unions), labels
...
if __name__=='__main__':
    ...
    relationship_labels = pickle.load(open('relationship_labels.pickle', 'rb'))
    learning_rate = args.learning_rate
    num_epochs = args.num_epochs
    batch_size = args.batch_size
    print("Learning Rate:", learning_rate)
    print("Batch Size:", batch_size)
    model = PredictPredicateNN(len(relationship_labels))
